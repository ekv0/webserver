...
其中Reactor在主线程运行，其他4种handler运行在worker线程。

线程池的目的就是为handler提供线程资源，同时最小化线程创建和销毁的开销。

<!-- 在使用线程池分配计算资源的情况下，每条连接都由且仅由一个线程处理，但不是某个时刻的所有连接都同时拥有一个线程资源，因为worker线程的总数在线程池初始化时就决定了，同时拥有线程资源的连接数不能超过这个数量。暂时不拥有计算资源的连接在线程池的任务队列中，等待被服务。 -->
